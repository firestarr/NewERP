  ...
  <div class="page-header">
        ...
        <div class="step-content">
          ...
          <div v-if="selectedFile" class="import-options">
            <label class="checkbox-label">
              ...
              Update existing purchase orders if duplicate PO numbers are found
            </label>
            <label class="checkbox-label">
              <input type="checkbox" v-model="validateVendors">
              <span class="checkmark"></span>
              Validate vendor codes before import
            </label>
            <label class="checkbox-label">
              <input type="checkbox" v-model="validateItems">
              <span class="checkmark"></span>
              Validate item codes before import
            </label>
          </div>
          ...
          <div v-if="selectedFile" class="import-actions">
            ...
            >
              ...
              {{ isImporting ? 'Importing...' : 'Import Purchase Orders' }}
            </button>
    ...
    <div v-if="isImporting" class="loading-overlay">
      <div class="loading-content">
        ...
        <h3>Importing Purchase Orders</h3>
  ...
  methods: {
    async downloadTemplate() {
      ...
      try {
        ...
        link.href = url;
        link.setAttribute('download', `purchase_order_template_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    downloadErrorReport() {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `purchase_order_import_errors_${new Date().toISOString().split('T')[0]}.txt`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
