    ...
    <div v-else class="rfq-details">
      ...
      <div class="detail-card">
        ...
        <div class="card-body">
          ...
          <div class="detail-row" v-if="rfq.reference_document">
            <div class="detail-group">
      ...
      <!-- **NEW: Selected Vendors Card** -->
      <div class="detail-card" v-if="hasSelectedVendors || rfq.status === 'draft'">
        <div class="card-header">
          <h2 class="card-title">Selected Vendors</h2>
        ...
        <div class="card-body">
          <div v-if="!hasSelectedVendors" class="empty-vendors">
            <div class="empty-icon">
              <i class="fas fa-users"></i>
            </div>
            <h3>No Vendors Selected</h3>
            <p v-if="rfq.status === 'draft'">
              This RFQ doesn't have any selected vendors yet. You can add vendors when sending the RFQ.
            </p>
            <p v-else>
              No vendors were selected for this RFQ.
            </p>
          ...
          <div v-else class="selected-vendors-list">
            ...
            >
              ...
              <div class="vendor-status">
                <div class="status-badge" :class="getVendorStatusClass(vendor.pivot.status)">
                  {{ capitalizeFirstLetter(vendor.pivot.status) }}
                </div>
                <div v-if="vendor.pivot.sent_at" class="sent-date">
                  Sent: {{ formatDateTime(vendor.pivot.sent_at) }}
                </div>
              </div>
            </div>

            <div class="vendors-actions" v-if="rfq.status === 'draft' && hasSelectedVendors">
              <router-link :to="`/purchasing/rfqs/${id}/send`" class="btn btn-primary">
                <i class="fas fa-paper-plane"></i> Send to {{ selectedVendors.length }} Vendor{{ selectedVendors.length > 1 ? 's' : '' }}
              </router-link>
            </div>
          </div>
        </div>
      ...
      <!-- **ENHANCED: Vendor Quotations Card** -->
      <div class="detail-card">
        <div class="card-header">
              ...
              <div class="quotation-body">
                <div class="quotation-items">
                  <div class="quotation-item-count">
                    {{ quotation.lines ? quotation.lines.length : 0 }} Items Quoted
                  </div>
...
export default {
  ...
  computed: {
    ...
    // **NEW: Computed for selected vendors**
    selectedVendors() {
      return this.rfq && this.rfq.selected_vendors ? this.rfq.selected_vendors : [];
    },
    hasSelectedVendors() {
  ...
  methods: {
    async loadRFQ() {
      ...
      try {
        ...
        if (response.data.status === 'success' && response.data.data) {
          ...
          // **NEW: Ensure selected_vendors is array**
          if (!Array.isArray(this.rfq.selected_vendors)) {
            this.rfq.selected_vendors = [];
          }

          // Ensure each quotation has a lines array
          ...
          console.log('RFQ loaded with selected vendors:', this.rfq.selected_vendors.length);
    ...
    getQuotationStatusClass(status) {
      switch (status) {
        ...
        case 'received': return 'status-submitted';
...
<style scoped>
/* Existing styles remain the same, plus new vendor-specific styles */
